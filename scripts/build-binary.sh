#!/usr/bin/env bash
#
# Build a standalone ticketbook binary via `bun build --compile`.
#
# Pipeline:
...
#   3. Run `bun build --compile` against bin/ticketbook.ts, which pulls in
#      the generated imports + the SKILL.md import attribute and emits a
#      single binary to dist/ticketbook-<target>
...
TARGET="${TICKETBOOK_TARGET:-bun-darwin-arm64}"
OUTFILE="dist/ticketbook-${TARGET#bun-}"
...
bun --filter @ticketbook/ui build
