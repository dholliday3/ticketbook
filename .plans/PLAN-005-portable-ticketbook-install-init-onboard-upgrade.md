...
| 4 | `ticketbook upgrade` / `--check` / `--json` with DI'd fetch + spawn | `74b78b4` |
...
- `version.ts` as a single source of truth, read by both `ticketbook upgrade` and the MCP handshake
...
- Once the first release exists, `ticketbook upgrade --check` starts returning real data instead of the current 404-from-empty-releases error.

## Thesis

Two independent pieces have to work for a user on a fresh repo to go from zero to a working ticketbook setup:

1. **A binary on their PATH.** Before this plan there wasn't one — the root `package.json` is `"private": true`, so `bunx ticketbook` failed, and nothing produced a distributable artifact. Phases 1–3 solved this via `bun build --compile` + GitHub Releases cross-compile + `scripts/install.sh`.

2. **A clean per-project init/onboard layer.** The part we undercooked originally. `initTicketbook` existed and was well-tested, but it conflated data-layer scaffolding with agent instruction delivery, and the agent instructions were write-once-if-missing — any update to the recommended workflow silently failed to reach projects that already initialized. Studying seeds (`~/workspace/resources/seeds`) surfaced a clearly better pattern: split `init` from `onboard`, wrap agent instructions in versioned HTML-comment markers, and surgically replace the bracketed section on update.

Seeds solves the *distribution* half via npm publish, which does not transfer to ticketbook — ticketbook is a Bun workspace monorepo with `workspace:*` deps, a React UI bundle in `packages/ui/dist/`, SQLite, and the MCP SDK, which together are too heavy to ship as source the way seeds does. But seeds' init/onboard/upgrade patterns are **distribution-agnostic** and transferred directly.

The plan had five phases. Phase 0 is the seeds-inspired per-project layer + two small multi-repo UX improvements; it shipped without any binary work. Phases 1–3 are the compiled-binary path. Phase 4 adds `ticketbook upgrade`. Each phase is independently shippable so nothing gated on the binary rabbit hole.
...
- `initTicketbook` in `packages/core/src/init.ts` scaffolds `.tasks/`, `.plans/`, `.docs/`, `.config.yaml`, `.counter`, `.mcp.json`, `.claude/skills/ticketbook/SKILL.md`, `.agents/skills/ticketbook/SKILL.md`, and `.gitignore` entries — idempotent, covered by 16 tests in `init.test.ts`
- Dev-mode detection in `init.ts` correctly dogfoods against the ticketbook source repo (writes `bun bin/ticketbook.ts --mcp` instead of the published command)
...
Shipped independently of any binary work. Goal: make the per-project init/onboard layer feel right *and* clean up the two rough edges that show up as soon as you run ticketbook across multiple repos, before any of this got frozen into a shipped binary.
...
- **TKTB-070** — Project-named MCP server. `.tasks/.config.yaml` gains an optional `name` field (auto-populated from `basename(projectRoot)` at init); `packages/server/src/mcp.ts` reads it via `resolveMcpServerName` and suffixes the MCP handshake name as `ticketbook-<name>`. Falls back to plain `"ticketbook"` when absent or malformed.
- **TKTB-071** — UI server port auto-increment. `bin/ticketbook.ts` defaults to 4242 and auto-increments on `EADDRINUSE` via `bindWithIncrementUsing<S>(tryBind, start, maxTries)` — a generic callback-style binder that preserves WebSocket type narrowing (smarter than an options-splat). Explicit `--port <N>` bypasses the retry loop.
- **TKTB-073** — Split init from onboard at the CLI layer. Delete `AGENTS_MD_CONTENT` + `writeAgentsMd` from `init.ts`; wire the `onboard` subcommand into `bin/ticketbook.ts` with `--check` / `--stdout` / `--json` flags.
- **TKTB-074** — README rewrite + `.claude-plugin/plugin.json` deletion.

### Phase 1 — Standalone binary ✅

`bun build --compile` with `with { type: "file" }` import attributes for asset embedding. SKILL.md is a single import in `bin/ticketbook.ts`; the 312-file `packages/ui/dist/` gets generated into `packages/server/src/embedded-ui.gen.ts` by `scripts/generate-embedded-ui.ts` before compile and reset to an empty stub after (via a `trap ... EXIT` in `scripts/build-binary.sh` so failed builds never leak).

Shipped: `4c6a31a PLAN-005 Phase 1: compile ticketbook to a standalone binary`
...
- `PUBLISHED_MCP_ENTRY` flipped from `bunx ticketbook` to `ticketbook` (the binary is on PATH after install.sh)
- 5 dynamic `await import()` calls in `bin/ticketbook.ts` converted to static imports so `--compile` can bundle them
...
curl one-liner installer modeled after plannotator's install.sh, stripped to ticketbook's surface area. OS/arch detection, version resolution via GH Releases API, SHA256 verification, install to `~/.local/bin/ticketbook`, PATH warning, global skill install via git sparse-checkout (`skills/ticketbook/` → `~/.claude/skills/ticketbook/` + `~/.agents/skills/ticketbook/`).
...
CLI wrapping in `bin/ticketbook.ts` has a try/catch so the pre-first-release 404 surfaces as a one-line error (`ticketbook upgrade failed: Failed to fetch latest release from GitHub: 404 Not Found`) rather than a stack trace, in both plain and JSON modes.

14 unit tests for upgrade + 1 version sync test. All zero-network via DI'd fetch.

Shipped: `74b78b4 PLAN-005 Phase 4: ticketbook upgrade command`
...
- **Marker name collision risk** — no observed collisions; the `<!-- ticketbook:start -->` / `<!-- ticketbook:end -->` delimiters are distinctive enough.
...
- **Plannotator reference files driving Phase 1–3:**
  ...
  - `~/workspace/resources/plannotator/scripts/install.sh` — installer structure (stripped for ticketbook)
  - `~/workspace/resources/plannotator/apps/hook/server/index.ts` — `with { type: "text" }` embed reference (ticketbook uses `file` instead because of multi-file dist)
